...
为什么说这是个抽象操作呢？因为这是 Javascript 内部才会使用的操作，我们不会显示调用到。当需要将对象转换为相应的基本类型值时，`ToPrimitive` 就会调用对象的内部方法 `[[DefaultValue]]` 来完成。

`ToPrimitive` 操作接收两个参数，一个是 input 需要转换的值，第二个是可选参数 hint 代表期望的转换类型。并且在调用 `[[DefaultValue]]` 的时候 hint 会传递过去。

这里我们首先只需要知道 `[[DefaultValue]]` 会调用 `valueOf()` 和 `toString()` 来完成基本类型值的转换。但是请注意：**`valueOf()` 和 `toString()` 的调用逻辑顺序并不是固定的取决于 hint 参数**，这个我们下面会讲到。
...
以上代码我们通常称为显示类型转换，这里面就包含 `ToString` 抽象操作，也就是把非字符串值转换为字符串的操作。
...
字面量对象的原型是 `Object.prototype`，`Object.prototype.toString()` 返回内部属性 [[Class]] 的值，那么结果就是 `[object Object]`。OK 没有问题



然后测试一下  `ToPrimitive`  的调用逻辑。来看下这段代码：
